...
- **结果缓存**：生成类节点内置持久化结果缓存，相同输入自动复用本地结果、跳过重复提交，避免重复计费（详见下方「生成结果缓存」章节）。
...
所有生成类节点（Seedance 1.0 / 1.5 / 2.x 视频、Seedream 3 / 4 / 5 图像）都支持**持久化结果缓存**：当「生成参数 + 参考素材内容」与某个历史成功任务完全一致时，直接复用已下载的本地结果文件重建输出，**不向方舟提交新任务，不产生额外费用**。

- **缓存判定**：缓存 key 由模型版本、归一化后的 prompt、实际 seed、分辨率、宽高比、时长、`generate_audio`、联网搜索等全部生成参数，以及参考图/视频/音频的内容哈希组成；路径、修改时间、节点 ID 不参与判定。开启「随机种子」时每次运行都会重新生成，不会误复用旧结果。
- **持久化**：结果文件与索引保存在插件目录下的 `result_cache/`（不写入 `output/`），TTL 默认 24 小时、上限 256 条，超期/超量自动清理；缓存写入采用「先复制文件、后原子写索引」，不会出现半成品命中。
- **输出等价**：命中时返回的 video/image tensor、last_frame、response JSON（含原始 task_id）与正常生成路径等价；批量模式下同样会按原行为保存到 `output/`。
- **开关**：每个生成节点都有 `use_result_cache` 布尔开关，**默认开启**；关闭后行为与旧版本完全一致（每次都正常提交）。草稿（draft）模式结果不缓存。
- **安全降级**：缓存文件缺失、损坏或索引异常时自动回退为正常生成，不影响主流程。

清理缓存：删除插件目录下的 `result_cache/` 文件夹即可（或等待 TTL 自动过期）。
